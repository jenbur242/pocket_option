...
Places trades 5 seconds before signal time to account for communication delay
Example: Signal at 00:38:00 → Trade at 00:37:55 → Executes on server at 00:38:00
"""
import os
import json
import time
            ...
            for _, row in signals_df.iterrows():
                try:
                    ...
                    try:
                        ...
                        # Calculate trade execution time (5 seconds before signal time for communication delay)
                        trade_datetime = signal_datetime - timedelta(seconds=5)
                    ...
                    signal = {
                        ...
                        'trade_datetime': trade_datetime,  # 5 seconds before signal for communication delay
                    ...
                    print(f"   Trade time:  {trade_datetime.strftime('%H:%M:%S')} (5s before for communication delay)")
        ...
        while current_step <= strategy.max_steps:
            ...
            try:
                ...
                if next_action['action'] == 'continue':
                    ...
                    await asyncio.sleep(1.0)  # Wait after error
                elif next_action['action'] == 'reset_after_max_loss':
                        ...
                        while (datetime.now() - start_time).total_seconds() < max_wait:
                            try:
                                ...
                                if remaining > 5:
                                    ...
                                    await asyncio.sleep(1.0)  # Reduce API calls
                                else:
                                    break
    ...
    async def execute_precise_trade(self, signal: Dict, amount: float) -> Tuple[bool, float]:
        ...
        try:
            ...
            if wait_seconds > 0:
                ...
                print(f"📝 {asset} {direction.upper()} → Execute exactly at signal time")
                
                # Wait with high precision for exact timing
                while wait_seconds > 0.1:  # Wait until we're within 100ms
                    if wait_seconds > 10:
                        await asyncio.sleep(1.0)  # Fast updates every 1s
                        current_time = datetime.now()
                        wait_seconds = (trade_time - current_time).total_seconds()
                        if wait_seconds > 10:
                            print(f"⏰ {wait_seconds:.0f}s until EXACT trade execution")
                    elif wait_seconds > 1:
                        # For 1-10 seconds, sleep in smaller increments
                        await asyncio.sleep(1.0)  # 1000ms increments
                        current_time = datetime.now()
                        wait_seconds = (trade_time - current_time).total_seconds()
                    else:
                        # For final second, use lower precision
                        await asyncio.sleep(1.0)  # 1000ms increments
                        current_time = datetime.now()
                        wait_seconds = (trade_time - current_time).total_seconds()
                
                # Final precision timing - execute within 1s of target time
                while True:
                    current_time = datetime.now()
                    remaining = (trade_time - current_time).total_seconds()
                    if remaining <= 1.0:  # Execute within 1s
                        break
                    await asyncio.sleep(1.0)  # 1s precision
                    ...
                    try:
                        ...
                        while (datetime.now() - start_time).total_seconds() < max_wait:
                            try:
                                ...
                                remaining = max_wait - elapsed
                                if remaining > 2:
                                    print(f"⏳ Trade active, checking again in 1.0s (remaining: {remaining:.0f}s)")
                                    await asyncio.sleep(1.0)
                                else:
                                    break
        ...
        try:
            ...
            while True:
                ...
                if not signals and not self.pending_immediate_trades:
                    ...
                    await asyncio.sleep(1.0)  # Check every 1 second for upcoming signals
                    continue
                ...
                if signals:
                    ...
                    if not ready_signals:
                        ...
                        await asyncio.sleep(1.0)  # Wait 1 second and check again
                        continue
                ...
                await asyncio.sleep(1.0)  # 1s check interval
...
async def main():
    ...
    print("⏰ Scheduled trades: exactly at signal time")
    ...
    while True:
        ...
        try:
            ...
            example_trade = "00:38:00"
            example_close = "00:38:59"
            print(f"   Signal Time: {example_signal}")
            print(f"   Trade Time:  {example_trade} (exactly at signal)")
            ...
            # Initialize trader with stop loss and take profit
